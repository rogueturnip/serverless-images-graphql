  ...
  try {
    await s3Client.send(
      new CopyObjectCommand({
        ...
        Key: destinationKey,
        ContentType: `image/${destinationKey.split(".").pop().toLowerCase()}`, // Adjust content type based on your requirements
      })
    );
  } catch (error) {
